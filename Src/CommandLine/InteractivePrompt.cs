namespace Microsoft.Formula.CommandLine
{
    ...
    // MIT licensed code adapted from https://github.com/sorrell/InteractivePrompt
    public static class InteractivePrompt
    {
        ...
        /// </summary>
        /// <param name="ci">The command interface object</param>
        public static void Run(CommandInterface ci)
        {
            List<string> completionList = new List<string> { "load", "list", "solve", "query", "proof" };
            string prompt = "[]> ";
            _prompt = prompt;
            ...
            while (true)
            {
                ...
                cmd = cmd.Trim();
                if (cmd == CommandInterface.ExitCommand ||
                    cmd == CommandInterface.ExitShortCommand)
                {
                    return;
                }
