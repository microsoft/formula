...
%bldexe%  .\Tools\Test\Test.sln /t:Clean /p:Configuration=Debug /verbosity:quiet /nologo
if %ERRORLEVEL% neq 0 (
  echo Could not clean test utility.
  exit /B 1
)

%bldexe%  .\Tools\Test\Test.sln /t:Build /p:Configuration=Debug /verbosity:quiet /nologo
if %ERRORLEVEL% neq 0 (
